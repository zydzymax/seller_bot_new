...
class InMemoryRateLimit:
    ...
    async def check_duplicate(self, update_id: int, ttl: int) -> bool:
        ...
        async with self.lock:
            ...
            expired_keys = [
                k for k, t in self.duplicates.items() if current_time - t > ttl
            ]
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, config: RateLimitConfig, redis_url: Optional[str] = None):
        ...
        self.excluded_paths: Set[str] = {"GET:/health", "GET:/healthz", "GET:/metrics"}
    ...
    async def _check_redis_rate_limit(
        ...
        try:
            ...
            member = f"{current_time_ms}:{time.monotonic_ns()}"

            pipe = self.redis_client.pipeline()
            pipe.zadd(key, {member: current_time_ms})
            pipe.zremrangebyscore(key, 0, current_time_ms - window_ms)
            pipe.zcard(key)
    ...
    async def _check_redis_duplicate(self, update_id: int) -> bool:
        ...
        try:
            ...
            result = await self.redis_client.set(
                key, "1", ex=self.config.dup_ttl, nx=True
            )
            return result is None  # None means key already existed

        except Exception as e:
            logger.warning(
                f"redis_duplicate_error error={str(e)} update_id={update_id}"
            )
            return await self.fallback.check_duplicate(update_id, self.config.dup_ttl)
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if content_length and int(content_length) > self.config.body_limit:
            logger.warning(
                f"request_body_too_large ip={client_ip} size_bytes={int(content_length)} limit_bytes={self.config.body_limit}"
            )
        ...
        if len(body) > self.config.body_limit:
            logger.warning(
                f"request_body_too_large_actual ip={client_ip} size_bytes={len(body)} limit_bytes={self.config.body_limit}"
            )
        ...
        if update_id:
            try:
                ...
                logger.warning(
                    f"duplicate_check_error error={str(e)} update_id={update_id}"
                )

        # Проверка IP rate limit
        try:
            ip_allowed, ip_metadata = await self._check_redis_rate_limit(
                f"rl:ip:{client_ip}", self.config.ip_burst
            )

            if not ip_allowed:
                log_data["limited"] = "ip"
                logger.warning(
                    f"ip_rate_limit_exceeded {log_data} count={ip_metadata.get('count')} limit={ip_metadata.get('limit')}"
                )
        ...
        if chat_id != "unknown":
            try:
                chat_allowed, chat_metadata = await self._check_redis_rate_limit(
                    f"rl:chat:{chat_id}", self.config.chat_burst
                )

                if not chat_allowed:
                    log_data["limited"] = "chat"
                    logger.warning(
                        f"chat_rate_limit_exceeded {log_data} count={chat_metadata.get('count')} limit={chat_metadata.get('limit')}"
                    )
                    ...
                    return JSONResponse(
                        status_code=429, content={"detail": "rate_limited"}
                    )
            except Exception as e:
                logger.warning(
                    f"chat_rate_limit_error error={str(e)} chat_id={chat_id}"
                )
        ...
        logger.debug(
            f"webhook_request_completed ip={client_ip} chat_id={chat_id} update_id={update_id} duration_ms={int(duration * 1000)}"
        )
...
def setup_middlewares(
    ...
    *,
    ...
    try:
        ...
        logger.info(
            f"rate_limit_middleware_setup_complete redis_available={REDIS_AVAILABLE} prometheus_available={PROMETHEUS_AVAILABLE} ip_burst={ip_burst} chat_burst={chat_burst} body_limit_mb={body_limit // 1024 // 1024}"
        )
