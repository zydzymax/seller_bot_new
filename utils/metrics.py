...
class PrometheusMetricsCollector:
    ...
    def __init__(
        self, redis_url: str = None, registry: Optional[CollectorRegistry] = None
    ):
    ...
    def _init_prometheus_metrics(self):
        ...
        self.crm_dlq_size = Gauge(
            "crm_dlq_size", "CRM DLQ size", registry=self.registry
        )
    ...
    def _init_base_metrics(self):
        ...
        base_metrics = [
            ...
            (
                "telegram_updates_total",
                MetricType.COUNTER,
                "Общее количество обработанных Telegram updates",
            ),
            (
                "telegram_updates_duration_seconds",
                MetricType.HISTOGRAM,
                "Время обработки Telegram updates в секундах",
            ),
            (
                "telegram_updates_errors_total",
                MetricType.COUNTER,
                "Количество ошибок обработки Telegram updates",
            ),
            # FSM метрики
            (
                "fsm_state_transitions_total",
                MetricType.COUNTER,
                "Количество переходов FSM по состояниям",
            ),
            (
                "fsm_current_sessions",
                MetricType.GAUGE,
                "Текущее количество активных сессий FSM",
            ),
            (
                "fsm_validation_errors_total",
                MetricType.COUNTER,
                "Количество ошибок валидации FSM",
            ),
            # LLM метрики
            (
                "llm_requests_total",
            ...
            (
                ...
                MetricType.COUNTER,
                "Количество созданных лидов в CRM",
            ),
            (
                "crm_contacts_created_total",
                MetricType.COUNTER,
                "Количество созданных контактов в CRM",
            ),
            (
                "crm_requests_duration_seconds",
                MetricType.HISTOGRAM,
                "Время выполнения CRM запросов",
            ),
            ...
            (
                ...
                MetricType.COUNTER,
                "Количество заблокированных запросов rate limiter",
            ),
            (
                "rate_limit_current_usage",
                MetricType.GAUGE,
                "Текущее использование rate limiter",
            ),
            # Idempotency метрики
            (
                "idempotency_duplicates_total",
            ...
            (
                ...
                MetricType.COUNTER,
                "Общее количество идемпотентных операций",
            ),
            # Системные метрики
            (
                "system_memory_usage_bytes",
    ...
    def increment_counter(
        ...
        try:
            # Legacy compatibility: increment_counter("name", 5.0)
            if isinstance(labels, (int, float)) and value == 1:
                value = float(labels)
                labels = None
            ...
            compat_name = f"{self.namespace}_{metric_name}"
            if compat_name in self.metrics:
                self.metrics[compat_name].value += value
                self.metrics[compat_name].timestamp = time.time()

            # Логирование performance метрики
            if "duration" in metric_name or "processing_time" in metric_name:
                log_performance(
                    f"counter_{metric_name}", int(value * 1000), labels=labels or {}
                )
    ...
    def set_gauge(self, metric_name: str, value: float, labels: Dict[str, str] = None):
        ...
        try:
            ...
            compat_name = f"{self.namespace}_{metric_name}"
            if compat_name in self.metrics:
                self.metrics[compat_name].value = float(value)
                self.metrics[compat_name].timestamp = time.time()
    ...
    def observe_histogram(
        ...
        try:
            ...
            # Логирование performance метрики
            log_performance(
                f"histogram_{metric_name}", int(value * 1000), labels=labels or {}
            )
    ...
    def record_duration(
        self, metric_name: str, duration_seconds: float, labels: Dict[str, str] = None
    ):
        """Legacy helper: записывает duration и увеличивает счетчик запросов."""
        self.observe_histogram(metric_name, duration_seconds, labels=labels)
        compat_metric_name = f"{self.namespace}_{metric_name}"
        if compat_metric_name in self.metrics:
            self.metrics[compat_metric_name].value = float(duration_seconds)
            self.metrics[compat_metric_name].timestamp = time.time()

        if metric_name.startswith("llm_"):
            compat_counter = f"{self.namespace}_llm_requests_total"
            if compat_counter in self.metrics:
                self.metrics[compat_counter].value += 1.0
                self.metrics[compat_counter].timestamp = time.time()
    ...
    def format_prometheus_metrics(self) -> str:
        ...
        # Legacy compatibility: include namespaced metrics snapshot.
        compat_lines = [f"{name} {metric.value}" for name, metric in self.metrics.items()]
        return text + "\n" + "\n".join(compat_lines) + "\n"
                    ...
                    try:
                        ...
                        if len(parts) >= 2:
                            ...
                            if "system_" in metric_name:
                                ...
                                for prefix in ["telegram_", "fsm_", "crm_"]
                            ):
    ...
    def decorator(func):
        async def async_wrapper(*args, **kwargs):
            ...
            try:
                ...
                collector.record_duration(metric_name, duration, error_labels)
                raise e
