            ...
            if key.lower() in cls.PII_FIELDS:
                ...
                if isinstance(value, str) and value:
                    if "@" in value:  # Email
                        ...
                        sanitized[key] = (
                            f"{value[:3]}***{value[-2:]}" if len(value) > 5 else "***"
                        )
                    else:  # Other text
...
class PerformanceLogFilter(logging.Filter):
    ...
    def filter(self, record):
        ...
        if (
            hasattr(record, "duration_ms")
            or "processing_time" in record.getMessage().lower()
        ):
...
def configure_logging(
    ...
    logging.basicConfig(
        level=getattr(logging, log_level), handlers=handlers, force=True
    )
