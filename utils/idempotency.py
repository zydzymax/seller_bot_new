...
class IdempotencyManager:
    ...
    async def mark_processing(
        ...
        try:
            ...
            if success:
                logger.info(f"Marked operation as processing", 
                           operation=operation, identifier=identifier)
            else:
                logger.warning(f"Operation already processing", 
                              operation=operation, identifier=identifier)
                              
            return success
            
        except Exception as e:
            logger.error(f"Error marking processing: {e}", 
                        operation=operation, identifier=identifier)
            return False
    ...
    async def mark_completed(
        ...
        try:
            ...
            logger.info(f"Marked operation as completed", 
                       operation=operation, identifier=identifier)
    ...
    async def mark_failed(
        ...
        try:
            ...
            logger.warning(f"Marked operation as failed", 
                          operation=operation, identifier=identifier, error=error)
    ...
    async def clear_operation(
        ...
        try:
            ...
            logger.info(f"Cleared operation", 
                       operation=operation, identifier=identifier)
        except Exception as e:
            logger.error(f"Error clearing operation: {e}", 
                        operation=operation, identifier=identifier)
    ...
    async def __call__(self, request, call_next=None):
        ...
        if check_result.is_duplicate:
            if check_result.status == IdempotencyStatus.COMPLETED:
                ...
                logger.warning(f"Previous request failed", 
                              operation=self.operation_name, 
                              identifier=identifier)
                return {
                ...
                logger.info(f"Request already processing", 
                           operation=self.operation_name, 
                           identifier=identifier)
                return {
