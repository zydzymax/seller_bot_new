...
# Настройки из ENV
MAX_INPUT_LENGTH = int(os.getenv("MAX_INPUT_CHARS", "1024"))
...
class InputSanitizer:
    ...
    def _compile_patterns(self):
        ...
        custom_patterns = self.business_rules.get("guards", {}).get(
            "role_reset_patterns", []
        )
    ...
    def remove_control_chars(self, text: str) -> str:
        ...
        cleaned = "".join(
            char
            for char in cleaned
            if unicodedata.category(char) not in ["Cc", "Cf", "Co"]
            or char in ["\n", "\r", "\t"]
        )
...
def sanitize_input_result(text: str) -> SanitizationResult:
    """
...
def sanitize_input(text: str) -> str:
    """
    Совместимость со старым API: возвращает строку.

    Для расширенного результата используйте sanitize_input_result().
    """
    from utils.input_sanitizer_old import sanitize_input as sanitize_input_legacy

    return sanitize_input_legacy(text)
