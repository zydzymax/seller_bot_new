...
QTY_FULL_RE = re.compile(
    ...
    + "]\d{3})*|\d+(?:[ \.,"
    + NBSP
    + "]\d{3})*)\s*(к|k|тыс\.?|тыс|тыщи)?\s*(шт|pcs)?",
    re.IGNORECASE,
)
...
def parse_quantity(text: str) -> int | None:
    ...
    for match in matches:
        ...
        compact = re.sub(r"[~≈\s" + NBSP + r"]", "", num_raw)
        # 5,000 / 5.000 -> 5000 (тысячный разделитель), 3,5 -> 3.5 (десятичный)
        if re.match(r"^\d+[.,]\d{3}$", compact):
            clean = compact.replace(",", "").replace(".", "")
        else:
            clean = compact.replace(",", ".")
        try:
            val = float(clean)
        except ValueError:
            continue
