...
class SalesStageAnalyzer:
    ...
    def analyze_stage(self, history: List[Dict]) -> str:
        ...
        user_messages = [
            msg["content"].lower() for msg in recent_messages if msg["role"] == "user"
        ]
        ...
        if stage_scores.get("objection", 0) > 0 and any(
            word in latest_message for word in ["не", "но", "однако"]
        ):
            return "objection"
    ...
    def analyze_emotion(self, history: List[Dict]) -> str:
        ...
        user_messages = [
            msg["content"].lower() for msg in history[-4:] if msg["role"] == "user"
        ]
        ...
        if emotion_scores.get("excited", 0) > 0 and (
            "!" in latest_message or "срочно" in latest_message
        ):
            return "excited"
    ...
    def extract_client_info(self, history: List[Dict]) -> Dict:
        ...
        timeframes = re.findall(
            r"(?:через|до|к)\s+\d+\s*(?:дня|дней|недель|месяца|месяцев)", text
        )
