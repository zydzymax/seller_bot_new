...
async def test_hot_patch():
    ...
    # Симуляция диалога
    conversations = [
        {"message": "Привет", "expect_contains": ["Алёна", "SoVAni", "менеджер"]},
        {"message": "футболки", "expect_contains": ["под ключ", "давальческое"]},
        {"message": "под ключ", "expect_contains": ["цвет"]},
        {"message": "3 цвета", "expect_contains": ["количество"]},
        {"message": "5000", "expect_contains": ["Резюмирую", "цвета"]},
    ]
    ...
    for i, step in enumerate(conversations, 1):
        ...
        try:
            ...
            if result.get("status") == "error":
                ...
                print("   ✅ Состояние изменилось корректно")
    ...
    try:
        ...
        result = await fm.process_message(
            user_info, None, {"message_id": 999}, {"update_id": 999}  # None message
        )
    ...
    try:
        result = await fm.process_callback_query(
            user_info,
            "change_quantity",
            {"update_id": 1000, "callback_query": {"id": "test_callback_123"}},
        )
