...
MODEL_MAP = {
    ...
    "gpt4": ("openai", "gpt-4-turbo"),  # legacy алиас
    # Legacy Anthropic aliases kept for compatibility with older tests/configs
    ...
    async def is_allowed(self, key: str) -> bool:
        ...
        try:
            ...
            if tokens >= 1.0:
                ...
                tokens -= 1.0
                await self.redis_client.hset(
                    bucket_key, mapping={"tokens": str(tokens), "last_refill": str(now)}
                )
                return True
            else:
                # Отклоняем запрос
...
class CircuitBreaker:
    ...
    def __init__(
        self, config: CircuitBreakerConfig, redis_client: redis.Redis, model: str
    ):
        self.config = config
        self.redis_client = redis_client
        self.model = model
    ...
    async def record_failure(self):
        ...
        try:
            ...
            await self.redis_client.hset(
                self._state_key,
                mapping={"failures": str(failures), "last_failure": str(time.time())},
            )
            await self.redis_client.expire(
                self._state_key, self.config.recovery_timeout
            )

            if failures >= self.config.failure_threshold:
                logger.warning(
                    f"Circuit breaker opened for {self.model} after {failures} failures"
                )
    ...
    async def get_fallback_response(self, prompt: str) -> str:
        ...
        return (
            "Извините, в данный момент у меня технические сложности. "
...
class LLMOrchestrator:
    ...
    def __init__(self, redis_url: str = "redis://localhost:6379/0"):
        ...
        self.rate_limiter = RateLimiter(
            self.redis_client, requests_per_second=2.0, burst_size=5
        )
        ...
        self.openai_client = httpx.AsyncClient(
            timeout=httpx.Timeout(
                self.config.get("timeouts", {}).get("read_ms", 60000) / 1000
            ),
            headers={
        ...
        # Backward-compatible alias для старых вызовов
        self.http_client = self.openai_client

        # Предпочитаем Responses API
        self.prefer_responses_api = True

        self.system_prompt_path = (
            Path(__file__).parent.parent / "config" / "system_prompt.md"
        )
        self.business_rules_path = (
            Path(__file__).parent.parent / "config" / "business_rules.yaml"
        )
    ...
    async def detect_available_models_openai(self, timeout: float = 3.0) -> List[str]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"OpenAI models detected: {len(self._available_models)} models (key: {key_preview})"
                )
        ...
        try:
            ...
            if cfg_default == "gpt-5":
                if "gpt-5" in available_models:
                    logger.info(
                        "Model chosen: gpt-5 -> gpt-5 (source: cfg, api: will_prefer_responses)"
                    )
                    return "gpt-5"
                else:
                    fallback = "gpt-4o"
                    logger.warning(
                        f"GPT-5 not available in account, falling back to {fallback} (source: auto)"
                    )
                    return fallback
            else:
                ...
                if model_id in available_models:
                    logger.info(
                        f"Model chosen: {cfg_default} -> {model_id} (source: cfg)"
                    )
                    return cfg_default
                else:
                    fallback = "gpt-4o"
                    logger.warning(
                        f"Model {cfg_default} not available, falling back to {fallback}"
                    )
                    return fallback
    ...
    async def _execute_with_jitter(
        self,
        func,
        *args,
        max_attempts: int = 3,
    ...
    async def run_completion(
        self, provider: str, model_id: str, messages: List[Dict[str, str]]
    ) -> Dict[str, Any]:
        """
        ...
        for attempt in range(max_attempts):
            try:
                if self.prefer_responses_api:
                    ...
                    try:
                        payload = {"model": model_id, "input": input_text.strip()}
                        ...
                        if response.status_code == 200:
                            data = response.json()
                            logger.info(
                                f"OpenAI Responses API success: model={model_id}, api=responses"
                            )
                            ...
                            if "choices" not in data and "output" in data:
                                ...
                                data = {
                                    "choices": [
                                        {
                                            "message": {
                                                "content": output_text or str(data)
                                            }
                                        }
                                    ],
                            ...
                            logger.warning(
                                f"Responses API error {response.status_code}: {response_text}"
                            )
                            # Продолжаем к chat/completions ниже
                    except Exception as e:
                        logger.warning(
                            f"Responses API exception: {e}, falling back to chat/completions"
                        )
                ...
                if response.status_code == 200:
                    data = response.json()
                    logger.info(
                        f"OpenAI Chat Completions success: model={model_id}, api=chat"
                    )
                    return data
                else:
                    response_text = response.text[:200]
                    raise Exception(
                        f"OpenAI API error {response.status_code}: {response_text}"
                    )
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(
                f"Ошибка загрузки конфигурации: {e}. Использую дефолтные значения."
            )
            return {
                "openai": LLMConfig(),
                "circuit_breaker": CircuitBreakerConfig(),
    ...
    def _load_system_prompt(self) -> str:
        ...
        try:
            ...
            return (
                "Ты — AI-помощник фабрики SoVAni. Помогай клиентам с заказами текстиля."
            )
    ...
    def _calculate_cost(
        self, tokens: int, model: str, config: CostTrackingConfig
    ) -> float:
        ...
        for attempt in range(config.max_retries):
            try:
                ...
                if attempt < config.max_retries - 1:
                    ...
                    logger.warning(
                        f"Попытка {attempt + 1} неуспешна: {e}. Повтор через {delay}s"
                    )
                    await asyncio.sleep(delay)
    ...
    def _generate_idempotency_key(
        self, user_prompt: str, context: Dict[str, Any], model: str
    ) -> str:
    ...
    async def generate_response(
        ...
        try:
            ...
            cfg_default = force_model or (
                openai_config.default_model
                if hasattr(openai_config, "default_model")
                else "gpt-4o"
            )
            ...
            idempotency_key = self._generate_idempotency_key(
                user_prompt, context, chosen_model
            )
            ...
            if not await circuit_breaker.can_execute():
                logger.warning(
                    f"Circuit Breaker открыт для {chosen_model}, используем fallback"
                )
                fallback_content = await circuit_breaker.get_fallback_response(
                    user_prompt
                )
                return LLMResponse(
            ...
            if context:
                context_str = (
                    f"Контекст диалога: {json.dumps(context, ensure_ascii=False)}"
                )
            ...
            if budget_check["exceeded"]:
                logger.warning(
                    f"Budget limit exceeded: {budget_check['limit_type']}",
                    **budget_check,
                )

                if (
                    budget_check["action"] == "truncate"
                    or budget_check["action"] == "aggressive_truncate"
                ):
                    # Обрезаем контекст
                    target_tokens = (
                        budget_check.get("limit", 4000) * 0.7
                    )  # 70% от лимита
                    messages = self._truncate_context(messages, int(target_tokens))
                    fallback_reason = f"context_truncated_{budget_check['limit_type']}"
                    logger.info(
                        f"Context truncated due to {budget_check['limit_type']} limit"
                    )
            ...
            try:
                ...
                response_data = await self._execute_with_jitter(
                    self.run_completion, provider, model_id, messages, max_attempts=3
                )
            ...
            cost = self._calculate_cost(
                tokens_used, chosen_model, self.config["cost_tracking"]
            )

            # Определение API, который использовался
            api_used = (
                "responses_api" if self.prefer_responses_api else "chat_completions"
            )
            ...
            await self.redis_client.setex(
                cache_key, self.config["openai"].cache_ttl, orjson.dumps(cache_data)
            )

            # Сохранение последнего ответа для fallback
            last_response_key = (
                f"llm:last_response:{hashlib.md5(user_prompt.encode()).hexdigest()}"
            )
    ...
    async def get_cost_metrics(self) -> Dict[str, Any]:
        ...
        try:
            total_cost = (
                await self.redis_client.get("llm:metrics:total_cost_rub") or "0"
            )
            total_tokens = (
                await self.redis_client.get("llm:metrics:total_tokens") or "0"
            )

            today = time.strftime("%Y-%m-%d")
            daily_cost = (
                await self.redis_client.get(f"llm:metrics:daily_cost:{today}") or "0"
            )
            daily_tokens = (
                await self.redis_client.get(f"llm:metrics:daily_tokens:{today}") or "0"
            )
