...
class OpenAIProvider(LLMProvider):
    def __init__(
        ...
        self.connector = aiohttp.TCPConnector(
            limit=5, limit_per_host=3, keepalive_timeout=300, enable_cleanup_closed=True
        )
    ...
    def _validate_request(self, request: LLMRequest):
        if request.model not in self.supported_models:
            raise ValueError(
                f"Модель {request.model} не поддерживается этим провайдером."
            )
    ...
    async def generate(self, request: LLMRequest) -> LLMResponse:
        ...
        while attempt < self.max_retries:
            try:
                ...
                async with aiohttp.ClientSession(
                    ...
                    async with session.post(
                        ...
                        if resp.status != 200:
                            ...
                            if resp.status >= 500 or resp.status in [
                                408,
                                424,
                                502,
                                503,
                        ...
                        logger.info(
                            "openai_success", latency_ms=latency_ms, attempt=attempt + 1
                        )
                        return LLMResponse(
                            content=content,
                            model=request.model,
                ...
                wait_time = min(2**attempt, 30)
                logger.warning(
                    f"OpenAI timeout, retry {attempt + 1}/{self.max_retries} in {wait_time}s"
                )
                ...
                wait_time = min(2**attempt, 30)
                logger.warning(
                    f"OpenAI network error: {str(e)}, retry {attempt + 1}/{self.max_retries}"
                )
                if attempt + 1 >= self.max_retries:
                    return LLMResponse(
                        content="",
                ...
                logger.exception(
                    "OpenAI API Exception (retrying)"
                    if attempt + 1 < self.max_retries
                    else "OpenAI API fatal error"
                )
                if attempt + 1 >= self.max_retries:
                    return LLMResponse(
                        content="",
