...
from llm.cache import RedisCache as ExternalRedisCache  # или in-memory
...
class LLMOrchestrator:
    def __init__(self, redis_url: Optional[str] = None):
        ...
        self.circuit_breakers = {
            k: AsyncCircuitBreaker(max_failures=100, reset_timeout=5)
            for k in self.providers
        }
        self.cache = ExternalRedisCache(redis_url) if redis_url else None
    ...
    async def generate_with_emotion_chain(
        self, prompt: str, context: Dict, timeout: int = 50
    ) -> str:
        """
        ...
        try:
            ...
            logic_context["system_prompt"] = context.get(
                "logic_system_prompt",
                "Ты профессиональный менеджер по продажам трикотажа. Отвечай четко, по делу, с конкретными фактами.",
            )

            logic_response = await self.generate(
                "logic", prompt, logic_context, timeout=timeout
            )
    ...
    async def generate(
        self, task_type: str, prompt: str, context: Dict, timeout: int = 35
    ) -> str:
        # 1. Кэш
        ...
        for name, provider in self.providers.items():
            ...
            tasks.append(
                self._call_provider(
                    cb, provider, prompt, context, name, timeout=vip_timeout
                )
            )
        ...
        for fut in done:
            try:
                ...
                logger.info(
                    "llm_success",
                    provider=getattr(fut, "provider_name", "unknown"),
                    task_type=task_type,
                    prompt_hash=hashlib.sha256(prompt.encode()).hexdigest(),
                )
                return sanitized
        ...
        try:
            ...
            result = await self._call_provider(
                cb, fallback, prompt, context, "fallback", timeout=timeout
            )
            sanitized = sanitize_output(result)
            if self.cache:
                await self.cache.set(cache_key, sanitized)
    ...
    async def _call_provider(
        ...
        try:
            fut = asyncio.create_task(
                cb.call(asyncio.wait_for(wrapper(), timeout=timeout))
            )
    ...
    def get_chain_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.chain_stats,
            "emotion_enhancement_rate": self.chain_stats["emotion_enhanced"]
            / total
            * 100,
