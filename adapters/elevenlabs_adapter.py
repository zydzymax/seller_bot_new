    ...
    async def set(self, text: str, voice_id: str, settings: Dict, audio_data: bytes):
        ...
        if self.redis:
            try:
                ...
                logger.info(
                    "audio_cached_redis", cache_key=cache_key[:8], size=len(audio_data)
                )
            except Exception as e:
        ...
        if len(audio_data) < 1024 * 1024:  # Только файлы < 1MB
            ...
            if len(self._memory_cache) > 50:
                oldest_key = min(
                    self._memory_cache.keys(), key=lambda k: self._memory_cache[k][0]
                )
...
class ElevenLabsAdapter:
    ...
    def __init__(
        ...
        # Connection pool для VPN оптимизации (инициализируется лениво)
        self.connector = None
    ...
    async def synthesize_speech(
        self, text: str, voice: str = "alena", priority: int = 5, **kwargs
    ) -> TTSResponse:
        """
        ...
        try:
            ...
            return TTSResponse(
                audio_data=response, duration_ms=duration_ms, voice_used=voice
            )
            ...
            if self.fallback_provider:
                try:
                    ...
                    fallback_audio = await self.fallback_provider.synthesize(
                        text, "nova"
                    )
                    return TTSResponse(
    ...
    async def _process_tts_request(self, request: TTSRequest) -> bytes:
        ...
        for attempt in range(self.max_retries):
            try:
                    ...
                    if hasattr(post_result, "__aenter__"):
                        async with post_result as response:
                            if response.status == 200:
                                ...
                                logger.info(
                                    "tts_success", size=len(audio_data), attempt=attempt + 1
                                )
                                return audio_data
                        ...
                        if hasattr(response, "__aenter__") and not isinstance(
                            getattr(response, "status", None), int
                        ):
                            async with response as nested_response:
                ...
                wait_time = 2**attempt
                logger.warning(
                    "request_timeout", attempt=attempt + 1, wait_time=wait_time
                )
    ...
    def _resolve_voice_id(self, voice: str) -> str:
        ...
        if (voice and len(voice) > 10 and not voice.lower().startswith("unknown")) or re.match(
            r"^[A-Za-z0-9]{20,}$", voice or ""
        ):
            return voice
    ...
    def _preprocess_text(self, text: str) -> str:
        ...
        if was_truncated and not result.endswith("..."):
    ...
    async def get_voices(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            async with aiohttp.ClientSession(
                ...
                get_result = session.get(url, headers=headers)
                if hasattr(get_result, "__aenter__"):
                    async with get_result as response:
                        if response.status == 200:
...
async def create_elevenlabs_adapter(
    ...
    adapter = ElevenLabsAdapter(
        api_key=api_key, redis_client=redis_client, fallback_provider=fallback_provider
    )
