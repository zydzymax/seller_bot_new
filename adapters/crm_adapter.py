...
class CRMAdapter:
    ...
    def __init__(self, redis_client: aioredis.Redis):
        ...
        logger.info("CRM Adapter инициализирован", 
    ...
    async def create_or_update_lead(
        ...
        try:
            ...
            if cached_result:
                logger.info("Lead уже обработан", 
                           external_id=external_id,
                           cached_result=cached_result.decode())
                return {"status": "cached", "data": json.loads(cached_result)}
            ...
            logger.info("Lead успешно создан",
            ...
            logger.error("Ошибка создания lead",
                        operation_id=operation_id,
                        error=str(e),
                        external_id=external_id)
    ...
    async def _create_or_find_contact(self, contact_data: ContactData) -> Optional[int]:
        ...
        try:
            ...
            logger.info("Контакт создан (заглушка)",
                       contact_id=contact_id,
            ...
            logger.error("Ошибка создания контакта",
                        error=str(e),
                        contact_data=asdict(contact_data))
            return None
    ...
    async def _create_lead(
        ...
        try:
            ...
            logger.info("Лид создан (заглушка)",
                       lead_id=lead_id,
                       contact_id=contact_id,
            ...
            logger.error("Ошибка создания лида",
                        error=str(e),
                        contact_id=contact_id,
                        order_details=asdict(order_details))
            return None
    ...
    async def _make_api_request(
        ...
        for attempt in range(self.max_retries):
            try:
                async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=30)) as session:
                    async with session.request(
                        ...
                        if response.status == 200 or response.status == 201:
                            ...
                            logger.warning("Rate limit, ожидание",
                ...
                logger.warning("Таймаут API запроса",
                             attempt=attempt + 1,
                             method=method,
                             endpoint=endpoint)
            except Exception as e:
                logger.error("Ошибка API запроса",
                           error=str(e),
                           attempt=attempt + 1,
                           method=method,
                           endpoint=endpoint)
    ...
    async def _add_to_dlq(self, failed_operation: Dict[str, Any]) -> None:
        ...
        try:
            await self.redis_client.lpush(
                self.dlq_key,
                json.dumps(failed_operation)
            )
            
            logger.info("Операция добавлена в DLQ",
                       operation_id=failed_operation.get("operation_id"),
                       error=failed_operation.get("error"))
                       
        except Exception as e:
            logger.error("Ошибка добавления в DLQ",
                        error=str(e),
                        operation_id=failed_operation.get("operation_id"))
        ...
        try:
            for _ in range(batch_size):
                ...
                try:
                    ...
                    if retry_count >= 3:
                        logger.warning("Операция превысила максимум попыток",
                    ...
                    if result["status"] == "success":
                        processed += 1
                        logger.info("DLQ операция успешно обработана",
                                   operation_id=operation.get("operation_id"))
                    else:
                    ...
                    logger.error("Ошибка обработки DLQ операции",
                               error=str(e))
                    errors += 1
            
            logger.info("DLQ обработка завершена",
                       processed=processed,
                       errors=errors)
            ...
            logger.error("Критическая ошибка обработки DLQ",
                        error=str(e))
            return {"error": str(e)}
    ...
    async def get_health_status(self) -> Dict[str, Any]:
        ...
        return {
            "status": "healthy" if redis_status == "healthy" and config_status == "healthy" else "degraded",
            "redis": redis_status,
