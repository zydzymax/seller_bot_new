...
class TochkaBankClient:
    ...
    async def init(self):
        ...
        if POSTGRES_DSN:
            try:
                self.db_pool = await asyncpg.create_pool(POSTGRES_DSN, min_size=1, max_size=3)
    ...
    async def create_statement(self, from_date: datetime, to_date: datetime) -> Optional[str]:
        ...
        try:
            ...
            response = await self.http_client.post(url, headers=self._get_headers(), json=data)
            
            if response.status_code == 200:
                result = response.json()
                statement_id = result.get('Data', {}).get('Statement', {}).get('statementId')
                logger.info(f"Statement created: {statement_id}")
                return statement_id
            else:
                logger.warning(f"Failed to create statement: {response.status_code} - {response.text[:200]}")
                return None
    ...
    async def get_statement(self, statement_id: str) -> List[Dict]:
        ...
        try:
            ...
            url = f"{TOCHKA_API_URL}/accounts/{account_encoded}/statements/{statement_id}"
    ...
    async def get_transactions(self, from_date: datetime, to_date: datetime) -> List[Dict]:
    ...
    async def find_invoice_payments(self, transactions: List[Dict]) -> List[Dict]:
        ...
        for tx in transactions:
            ...
            payer = tx.get('debtorAccount', {}).get('name', '') or tx.get('debtorAgent', {}).get('name', '')
    ...
    async def process_payment(self, payment_info: Dict) -> bool:
        ...
        async with self.db_pool.acquire() as conn:
            ...
            await conn.execute("""
                ...
                WHERE id=$6
            """, plan, expires_at, ai_seller, hos, amount, invoice['license_id'])
            
            await conn.execute("UPDATE widget_invoices SET status='paid', paid_at=NOW() WHERE id=$1", invoice['id'])
    ...
    async def run_forever(self):
        ...
        await self.send_notification("🏦 Мониторинг Точка Банк запущен\nСчёт: " + self.account_id[:20] + "...")
