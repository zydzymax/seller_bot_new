...
def _verify_admin_key(api_key: Optional[str]) -> None:
...
def generate_license_key() -> str:
    ...
    key = (
        f"SW-{random_part[:8].upper()}-{random_part[8:16].upper()}-{timestamp.upper()}"
    )
    return key
...
async def register_license(request: LicenseRequest):
    ...
    async with pool.acquire() as conn:
        ...
        existing = await conn.fetchrow(
            "SELECT * FROM widget_licenses WHERE account_id = $1", request.account_id
        )
...
async def verify_license(license_key: str):
    ...
    async with pool.acquire() as conn:
        license_data = await conn.fetchrow(
            "SELECT * FROM widget_licenses WHERE license_key = $1", license_key
        )

        if not license_data:
            return LicenseVerifyResponse(valid=False, message="Лицензия не найдена")

        # Check status
        if license_data["status"] == "cancelled":
            return LicenseVerifyResponse(valid=False, message="Лицензия отменена")
...
async def verify_by_account(account_id: str):
    ...
    async with pool.acquire() as conn:
        license_data = await conn.fetchrow(
            "SELECT * FROM widget_licenses WHERE account_id = $1", account_id
        )

        if not license_data:
            return LicenseVerifyResponse(
                valid=False, message="Лицензия не найдена. Установите виджет заново."
            )
...
async def activate_plan(license_key: str, plan: Plan, months: int = 1):
    """
...
async def admin_list_licenses(api_key: str = Header(None, alias="X-Admin-Key")):
    ...
    async with pool.acquire() as conn:
        licenses = await conn.fetch(
            "SELECT * FROM widget_licenses ORDER BY created_at DESC LIMIT 100"
        )

        return {"count": len(licenses), "licenses": [dict(row) for row in licenses]}
