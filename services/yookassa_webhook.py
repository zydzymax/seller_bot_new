...
# Plans configuration
PLANS = {
    'ai_seller': {'price': 1990, 'days': 30, 'ai_seller': True, 'head_of_sales': False, 'name': 'AI Seller'},
    'head_of_sales': {'price': 2990, 'days': 30, 'ai_seller': False, 'head_of_sales': True, 'name': 'Head of Sales'},
    'suite': {'price': 3990, 'days': 30, 'ai_seller': True, 'head_of_sales': True, 'name': 'Suite'}
}
...
async def process_payment(payment_data: dict) -> bool:
    ...
    try:
        ...
        if not license_id:
            ...
            if not email:
                await send_telegram(f"⚠️ Платёж без license_id!\nID: {payment_id}\nСумма: {amount}₽")
                return False
        ...
        async with pool.acquire() as conn:
            ...
            if license_id:
                ...
                license_data = await conn.fetchrow(
                    "SELECT * FROM widget_licenses WHERE email = $1 ORDER BY created_at DESC LIMIT 1", email
                )
            
            if not license_data:
                logger.error(f"License not found for payment {payment_id}")
                await send_telegram(f"⚠️ Лицензия не найдена!\nPayment: {payment_id}\nEmail: {email}")
                await pool.close()
                return False
            ...
            await conn.execute("""
                ...
                WHERE id = $6
            """, plan, new_expires, plan_info['ai_seller'], plan_info['head_of_sales'], 
                amount, license_data['id'])
            ...
            """, license_data['id'], payment_id, amount, plan)
        ...
        if self.path == '/api/payment/webhook' or self.path == '/webhook':
            ...
            try:
                ...
                if event == 'payment.succeeded':
                    ...
                    asyncio.run(send_telegram(f"❌ Платёж отменён: {payment.get('id')}"))
