...
AMOCRM_REDIRECT_URI = os.getenv(
    "AMOCRM_REDIRECT_URI", "https://saleswhisper.pro/api/amocrm/callback"
)
WIDGET_SECRET_KEY = os.getenv("AMOCRM_WIDGET_SECRET", "")
APP_ENV = os.getenv("APP_ENV", "development").lower()
ALLOW_INSECURE_WEBHOOK = (
    os.getenv("AMOCRM_ALLOW_INSECURE_WEBHOOK", "false").lower() == "true"
)
...
class TokenStorage:
    ...
    def __init__(self):
        redis_url = os.getenv("REDIS_ADDR") or os.getenv("REDIS_URL")
        self.redis_client = aioredis.from_url(redis_url) if redis_url else None
        self.redis_prefix = os.getenv("AMOCRM_TOKENS_PREFIX", "amocrm:oauth:tokens")

    def _redis_key(self, account_id: str) -> str:
        return f"{self.redis_prefix}:{account_id}"

    def _build_payload(self, account_id: str, tokens: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def save_tokens(self, account_id: str, tokens: Dict[str, Any]) -> None:
        """Save tokens for account"""
        payload = self._build_payload(account_id, tokens)
        _token_storage[account_id] = payload

        if self.redis_client:
            try:
                redis_payload = dict(payload)
                redis_payload["expires_at"] = payload["expires_at"].isoformat()
                ttl_seconds = max(int(tokens.get("expires_in", 86400)) + 3600, 3600)
                ...
                logger.warning("Redis token save failed for account=%s: %s", account_id, str(e))

        logger.info("Tokens saved for account %s", account_id)

    async def get_tokens(self, account_id: str) -> Optional[Dict[str, Any]]:
        """Get tokens for account"""
        if self.redis_client:
            try:
                raw = await self.redis_client.get(self._redis_key(account_id))
                if raw:
                    parsed = json.loads(raw)
                    parsed["expires_at"] = datetime.fromisoformat(parsed["expires_at"])
                    _token_storage[account_id] = parsed
                    return parsed
            except Exception as e:
                logger.warning(
                    "Redis token read failed for account=%s: %s", account_id, str(e)
                )

        return _token_storage.get(account_id)

    async def delete_tokens(self, account_id: str) -> None:
        ...
        if self.redis_client:
            try:
                await self.redis_client.delete(self._redis_key(account_id))
            except Exception as e:
                logger.warning(
                    "Redis token delete failed for account=%s: %s", account_id, str(e)
                )

        logger.info("Tokens deleted for account %s", account_id)

    async def list_accounts(self) -> list:
        """List all installed accounts"""
        account_ids = set(_token_storage.keys())
        if self.redis_client:
            try:
                ...
                async for key in self.redis_client.scan_iter(match=pattern):
                    ...
                    account_ids.add(key_str.rsplit(":", 1)[-1])
            except Exception as e:
                logger.warning("Redis token listing failed: %s", str(e))
...
def verify_webhook_signature(
    account_id: str, timestamp: int, received_hash: str
) -> bool:
    """Verify webhook signature from AmoCRM"""
    if not WIDGET_SECRET_KEY:
        # Fail-closed в production. Небезопасный bypass возможен только вручную
        # и только вне production.
        if APP_ENV in {"production", "prod"} or not ALLOW_INSECURE_WEBHOOK:
            logger.error(
                "Widget secret key is not configured; rejecting webhook in env=%s",
                APP_ENV,
            )
            return False

        logger.warning(
            "Widget secret key not configured; insecure webhook mode is enabled"
        )
        return True
...
async def refresh_access_token(account_id: str) -> Optional[Dict[str, Any]]:
    ...
    async with httpx.AsyncClient() as client:
        ...
        if response.status_code != 200:
            logger.error(
                "Token refresh failed for account=%s status=%s body=%s",
                account_id,
                response.status_code,
                response.text,
            )
...
async def oauth_install(
    ...
    logger.info("OAuth install initiated, redirecting to AmoCRM")
...
async def oauth_callback(
    ...
    if error:
        logger.error(
            "OAuth error: error=%s description=%s", error, error_description or ""
        )
        return HTMLResponse(
            content=f"""
            <html>
    ...
    try:
        ...
        logger.warning("Failed to parse referer: %s", str(e))
    ...
    try:
        ...
        logger.info("Widget installed account=%s domain=%s", account_id, domain)
...
async def webhook_handler(request: Request):
    ...
    # Verify signature
    if not verify_webhook_signature(account_id, timestamp_int, signature):
    ...
    if event_type == "install":
        logger.info("Widget installed via webhook account=%s", account_id)
        return {"status": "ok", "message": "Installation confirmed"}

    elif event_type == "uninstall":
        await token_storage.delete_tokens(account_id)
        logger.info("Widget uninstalled account=%s", account_id)
        return {"status": "ok", "message": "Uninstallation confirmed"}

    elif event_type == "settings_update":
        logger.info("Widget settings updated account=%s", account_id)
        return {"status": "ok", "message": "Settings updated"}

    else:
        logger.info(
            "Unknown webhook event=%s account=%s", event_type, account_id or "-"
        )
        return {"status": "ok"}
...
async def get_account_status(account_id: str):
    ...
    return {
        ...
        "domain": tokens.get("domain", ""),
        "expires_at": (
            tokens.get("expires_at", "").isoformat()
