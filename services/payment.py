...
async def create_yookassa_payment(amount: int, description: str, metadata: dict) -> dict:
...
async def create_payment(request: PaymentRequest):
    ...
    async with pool.acquire() as conn:
        ...
        if not license_data:
            ...
            """, license_key, request.domain.split('.')[0], request.domain, request.email, request.company)
            
            license_id = await conn.fetchval("SELECT id FROM widget_licenses WHERE license_key = $1", license_key)
        else:
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            """, license_id, payment['id'], request.price, request.plan, metadata)
    ...
    async with pool.acquire() as conn:
        ...
        if not license_data:
            ...
            await conn.execute("""
                INSERT INTO widget_licenses (license_key, account_id, account_domain, email, company_name, plan, status)
                VALUES ($1, $2, $3, $4, $5, 'pending', 'pending')
            """, license_key, request.domain.split('.')[0], request.domain, request.email, request.company_name)
        ...
        invoice_id = await conn.fetchval("""
            ...
            RETURNING id
        """, license_id, invoice_number, amount, request.plan, request.months,
            request.company_name, request.inn, request.kpp, request.legal_address, request.email, request.telegram)
...
async def confirm_invoice_payment(
    ...
    async with pool.acquire() as conn:
        invoice = await conn.fetchrow(
            "SELECT * FROM widget_invoices WHERE id = $1",
            invoice_id
        )
        ...
        # Get license
        license_data = await conn.fetchrow(
            "SELECT * FROM widget_licenses WHERE id = $1",
            invoice['license_id']
        )
        ...
        if current_expires and current_expires > now:
            expires_at = current_expires + timedelta(days=plan_info.get('days', 30) * months)
        else:
        ...
        await conn.execute("""
            ...
            WHERE id = $6
        """, plan, expires_at, plan_info.get('ai_seller', True), 
            plan_info.get('head_of_sales', True), invoice['amount'], invoice['license_id'])
