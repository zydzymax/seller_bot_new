...
def _extract_first_int(text: str) -> Optional[int]:
    # DEPRECATED: Use parse_quantity from utils.text_processing instead
...
class BusinessRulesValidator:
    ...
    def validate_moq(self, total_qty: int, colors_count: int, work_scheme: str = "turnkey") -> Dict[str, Any]:
        """Валидация минимального заказа (MOQ)"""
        if work_scheme == "client_materials":
            moq_per_color = self.rules.get('moq', {}).get('client_materials_per_color', 300)
        else:
...
class FlowManager:
    ...
    async def get_context(self, chat_id: int) -> DialogContext:
        ...
        import time
        context = DialogContext(user_id=0, chat_id=chat_id, created_at=time.time(), updated_at=time.time())
        await self._check_greeting_today(context)
        return context
    ...
    async def _check_greeting_today(self, context: DialogContext):
        ...
        try:
            ...
            if await self.redis_client.get(greet_key):
                ...
                tomorrow = datetime.now(moscow_tz).replace(hour=0, minute=0, second=0, microsecond=0) + timedelta(days=1)
    ...
    async def send_telegram_message(self, chat_id: int, text: str, update_id: int = None, keyboard: list = None, inline: bool = False) -> bool:
        ...
        if update_id is not None:
            try:
                ...
                if not can_send:
                    logger.warning("duplicate_turn_suppressed", chat_id=chat_id, update_id=update_id)
                ...
                if is_duplicate:
                    ...
                    try:
                        if "общее количество" in text:
                            ...
                            logger.info("anti_repeat", original_text=True, variant=text[-2:])
                    except Exception:
                        pass
        ...
        try:
            ...
            if keyboard:
                if inline:
                    ...
                    reply_markup = InlineKeyboardMarkup([[InlineKeyboardButton(btn, callback_data=btn) for btn in row] for row in keyboard])
                else:
    ...
    def _detect_color_intent(self, text: str) -> bool:
        ...
        color_markers = [
            'цвет', 'цвета', 'цветов', 'расцветка', 'расцветки', 'вариант', 'варианта',
            'оттенок', 'оттенка', 'тон', 'тона', 'колер', 'колора'
        ]
        
        change_markers = [
            'давайте', 'сделаем', 'пусть будет', 'хочу', 'нужно', 'лучше',
            'исправляю', 'поправляю', 'меняю на', 'нет', 'не'
        ]
    ...
    def extract_order_data(self, message_text: str, context: DialogContext) -> Dict[str, Any]:
        ...
        if context.current_state == FlowState.QUANTITY_COLORS:
            ...
            maybe_n = _extract_first_int(message_text)  # используем оригинальный текст для чисел
        ...
        color_keywords = ['красн', 'белый', 'черн', 'син', 'зелен', 'желт', 'сер', 'беж', 'розов', 'фиолет', 'корич', 'оранж']
        found_colors = []
        ...
        if any(word in text_lower for word in ['распределение', 'по цветам', 'остальные по']):
    ...
    async def process_fsm_logic(self, context: DialogContext, message_text: str) -> Dict[str, Any]:
        ...
        if current_state == FlowState.GREETING:
            ...
            if extracted_data.get('product_type') or context.product_type:
                ...
                await self.transition_to_state(context, FlowState.WORK_SCHEME, "product_specified")
                result["state_changed"] = True
            ...
            if mode:
                ...
                await self.transition_to_state(context, FlowState.QUANTITY_COLORS, "work_scheme_specified")
                result["state_changed"] = True
                
                # Короткое подтверждение + следующий вопрос
                confirm = "Записала режим: под ключ ✅" if mode == "pod_kluch" else "Записала режим: давальческое сырьё ✅"
                ...
                if hasattr(context, '_repeat_cache') and context._repeat_cache.get(repeat_key) == message_text:
            ...
            if extracted_data.get('product_type'):
                ...
                await self.transition_to_state(context, FlowState.WORK_SCHEME, "product_redefined")
                result["state_changed"] = True
            elif extracted_data.get('total_quantity') and extracted_data.get('colors_count'):
                ...
                moq_result = self.rules_validator.validate_moq(
                    context.total_quantity, context.colors_count, context.work_scheme or "turnkey"
                )
            ...
            if extracted_data.get('total_quantity') or extracted_data.get('colors_count'):
                ...
                if total and colors:
                    moq_result = self.rules_validator.validate_moq(total, colors, context.work_scheme or "turnkey")
                    if moq_result["valid"]:
    ...
    async def _get_slot_based_response(self, context: DialogContext, message_text: str) -> Optional[dict]:
            ...
            if context.colors_count and context.total_quantity:
                ...
                if qty_per_color >= min_qty:
                    ...
                    if should_offer_handoff(context):
                        summary_text = shape(t("summary", qty=context.total_quantity, colors=context.colors_count), "summary")
                        return {"text": summary_text, 
                               "keyboard": [["Изменить количество", "Изменить цвета"], ["Связать с менеджером"]],
                               "inline": True}
                    else:
                        return {"text": shape(t("summary", qty=context.total_quantity, colors=context.colors_count), "summary"), 
                               "keyboard": [["Изменить количество", "Изменить цвета"], ["Продолжить"]],
                               "inline": True}
                else:
                    return {"text": shape(f"При {context.colors_count} цветах получается {qty_per_color} шт на цвет/модель, но " + t("confirm.moq_hint", moq=min_qty), "ask")}
        ...
        if missing_slot == Slot.PRODUCT_TYPE:
            ...
            if context.work_scheme:
                min_qty = "1000" if context.work_scheme == "turnkey" else "300"
                scheme_name = "под ключ" if context.work_scheme == "turnkey" else "давальческое сырьё"
                text = f"Сколько цветов планируете? (1–5)\nВарианты: 1 • 2 • 3 • 4 • 5"
            ...
            try:
                should_oops_result = asyncio.create_task(should_oops(chat_id, "ask.total_qty"))
            ...
            if should_oops_flag:
                ...
                return {"text": shape(oops_response["text"], "ask"), "keyboard": oops_response["keyboard"]}
            ...
            if context.colors_count and context.work_scheme:
                min_total = context.colors_count * (1000 if context.work_scheme == "turnkey" else 300)
                text = t("confirm.colors_saved", colors=context.colors_count) + " " + t("ask.total_qty") + f" Минимум {min_total} шт. @v{variant}"
                return {"text": shape(text, "ask")}
            else:
                return {"text": shape(t("ask.total_qty") + f" @v{variant}", "ask")}
                
        elif missing_slot == Slot.FABRIC_COMPOSITION:
            return {"text": "Какой состав ткани планируете? (хлопок, полиэстер, футер и т.д.)"}
            
        elif missing_slot == Slot.CONTACT_PHONE:
            return {"text": "Отлично! Оставьте, пожалуйста, ваш номер телефона для связи."}
    ...
    async def process_message(
        ...
        try:
            ...
            if context.current_state in [FlowState.GREETING, FlowState.PRODUCT_INQUIRY, FlowState.WORK_SCHEME]:
                ...
                if fastlane_decision(slots):
                    ...
                    if slots.get("supply_mode"):
                        context.work_scheme = "turnkey" if "под ключ" in slots["supply_mode"] else "client_materials"
                    if slots.get("colors"):
                    ...
                    await self.transition_to_state(context, FlowState.QUANTITY_COLORS, "fastlane")
                    
                    # Формируем ответ с недостающими полями
                    missing_fields = []
                    if not context.total_quantity: missing_fields.append("количество")
                    if not context.colors_count: missing_fields.append("количество цветов") 
                    if not context.work_scheme: missing_fields.append("схему работы")
                    
                    if missing_fields:
                        response_text = f"Отлично! Понял: {context.product_type or 'изделие'}"
                        if context.total_quantity: response_text += f", {context.total_quantity} шт"
                        response_text += f". Уточните: {', '.join(missing_fields)}."
                        
                        await self.send_telegram_message(chat_id, shape(response_text, "ask"), update_id)
                        await self.save_context(context, update_id)
                        return {"state": context.current_state.value, "response_sent": True, "fastlane": True}
...
Как будем корректировать заказ?"""
                        ...
                        await self.send_telegram_message(chat_id, response_text, update_id)
                        await self.save_context(context, update_id)
            ...
            if message_text == "Связать с менеджером":
                ...
                return {"state": context.current_state.value, "response_sent": True, "handoff": True}
            ...
            if not fsm_result.get("state_changed") and not fsm_result.get("work_scheme_confirmed"):
                # СИСТЕМНАЯ ЛОГИКА НА ОСНОВЕ СЛОТОВ - идемпотентная, без повторов
                slot_response = await self._get_slot_based_response(context, message_text)
                
                if slot_response:
                    # Используем системный ответ на основе слотов
                    text = slot_response.get("text", slot_response) if isinstance(slot_response, dict) else slot_response
                    keyboard = slot_response.get("keyboard") if isinstance(slot_response, dict) else None
                    inline = slot_response.get("inline", False) if isinstance(slot_response, dict) else False
                    await self.send_telegram_message(chat_id, text, update_id, keyboard, inline)
                    await self.save_context(context, update_id)
            ...
            llm_context = {
                ...
                "flags": {
                    "greeted_today": getattr(context, 'greeted_today', False)
                },
                "business_rules": {
            ...
            # Отправка ответа
            response_sent = await self.send_telegram_message(chat_id, llm_response.content, update_id)
            ...
            try:
                ...
                basic_fallback = "Технический сбой. Начните, пожалуйста, заново с описания изделия."
    ...
    async def process_callback_query(
        ...
        try:
            ...
            if callback_data == "change_quantity":
                ...
                await self.transition_to_state(context, FlowState.QUANTITY_COLORS, "callback_change")
                ...
                await self.transition_to_state(context, FlowState.FABRIC_DETAILS, "callback_continue")
            ...
            if response_text:
                await self.send_telegram_message(chat_id, response_text, update.get('update_id'))
