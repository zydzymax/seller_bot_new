...
class FlowManager:
    ...
    async def process(self, user_id, message, context):
        ...
        if should_greet:
            context.user_data["last_greeting_date"] = (
        ...
        logger.info(
            ...
            stage=stage,
            emotion=emotion,
            needs_step=needs_progress["current_step"],
            qualified=needs_progress["is_qualified"],
        )

        # Формируем контекст для ChatGPT
        gpt_context = self._build_gpt_context(
            stage, emotion, history, needs_progress, should_greet, client_info
        )
    ...
    def _build_gpt_context(
        ...
        gpt_prompt = build_prompt(
            llm="gpt-4", stage=stage, emotion=emotion, history=history[-8:]
        )

        # Добавляем информацию о прогрессе выявления потребностей
        if needs_progress["current_step"] < len(self.needs_assessment_steps):
            current_step_name = self.needs_assessment_steps[
                needs_progress["current_step"]
            ]
    ...
    def _auto_update_needs_progress(
        self, user_message: str, bot_response: str, context: Dict
    ) -> None:
        ...
        if step_key == "product_type":
            if any(
                ...
                for word in ["пижам", "худи", "футболк", "костюм", "свитшот"]
            ):
                step_data = user_message
        elif step_key == "work_format":
            if any(
                word in user_msg_lower
                for word in ["ключ", "давальческ", "сырье", "ткань"]
            ):
                step_data = user_message
        elif step_key == "quantity":
            if any(
                word in user_msg_lower
                for word in ["штук", "единиц", "1000", "500", "300"]
            ):
                step_data = user_message
            ...
            if any(
                word in user_msg_lower
                for word in ["есть", "готов", "нет", "строить", "лекала"]
            ):
                step_data = user_message
            ...
            if any(
                word in user_msg_lower
                for word in ["телефон", "номер", "связи", "+7", "8-", "89"]
            ):
                step_data = user_message
