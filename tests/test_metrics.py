...
class TestMetricsCollector:
    ...
    def test_init_base_metrics(self, metrics_collector):
        ...
        assert (
            "sovani_ai_seller_fsm_state_transitions_total" in metrics_collector.metrics
        )
    ...
    def test_increment_counter(self, metrics_collector):
        ...
        initial_value = metrics_collector.metrics[
            f"sovani_ai_seller_{metric_name}"
        ].value
    ...
    def test_set_gauge(self, metrics_collector):
        ...
        stored_value = metrics_collector.metrics[
            f"sovani_ai_seller_{metric_name}"
        ].value
    ...
    def test_record_duration(self, metrics_collector):
        ...
        initial_count = metrics_collector.metrics[
            "sovani_ai_seller_llm_requests_total"
        ].value

        metrics_collector.record_duration(metric_name, test_duration)

        # Проверяем что время записано
        stored_duration = metrics_collector.metrics[
            f"sovani_ai_seller_{metric_name}"
        ].value
        assert stored_duration == test_duration

        # Проверяем что счетчик увеличился
        new_count = metrics_collector.metrics[
            "sovani_ai_seller_llm_requests_total"
        ].value
    ...
    async def test_collect_system_metrics(self, metrics_collector):
        ...
        with patch("psutil.virtual_memory") as mock_memory, patch(
            ...
            memory_metric = metrics_collector.metrics[
                "sovani_ai_seller_system_memory_usage_bytes"
            ]
            assert memory_metric.value == 1024 * 1024 * 512

            cpu_metric = metrics_collector.metrics[
                "sovani_ai_seller_system_cpu_usage_percent"
            ]
    ...
    def test_get_metrics_json(self, metrics_collector):
        ...
        assert (
            json_data["metrics"]["sovani_ai_seller_fsm_current_sessions"]["value"] == 5
        )
...
class TestHealthChecker:
    ...
    async def test_check_redis_health_success(self, health_checker):
        ...
        with patch.object(
            health_checker.metrics_collector.redis_client,
            "ping",
    ...
    async def test_run_full_health_check(self, health_checker):
        ...
        with patch.object(
            ...
            mock_system.return_value = MagicMock(
                status="healthy", component="system", details={}, response_time_ms=10
            )
            mock_redis.return_value = MagicMock(
                status="healthy", component="redis", details={}, response_time_ms=5
            )
