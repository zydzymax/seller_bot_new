...
• Время: {context.created_at or 'сейчас'}"""
        ...
        if context.contact_phone:
            # Кнопка для звонка
            phone_clean = context.contact_phone.replace('+', '').replace(' ', '').replace('-', '')
            manager_keyboard = [[{"text": "📞 Позвонить", "url": f"tel:+{phone_clean}"}]]
        
        try:
            await send_message(MANAGER_CHAT_ID, manager_msg, manager_keyboard, inline=True)
        except Exception as e:
...
def should_offer_handoff(context) -> bool:
    ...
    if context.total_quantity and context.total_quantity >= 3000 and context.work_scheme:
        return True
