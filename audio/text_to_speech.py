...
class OpenAITTS(TTSProvider):
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        self.model = os.getenv("TTS_MODEL", "gpt-4o-mini-tts")
        self.fallback_model = os.getenv("TTS_FALLBACK_MODEL", "tts-1-hd")
        self.default_instructions = os.getenv("TTS_INSTRUCTIONS", "").strip()
    ...
    def get_available_voices(self) -> Dict[str, str]:
        ...
        return {
            ...
            "ash": "Выразительный голос (Ash)",
            "ballad": "Спокойный голос (Ballad)",
            "coral": "Мягкий женский голос (Coral)",
            "echo": "Мужской голос (Echo)",
            "fable": "Британский мужской голос (Fable)",
            "onyx": "Глубокий мужской голос (Onyx)",
            "nova": "Женский голос (Nova)",
            "sage": "Низкий спокойный голос (Sage)",
            "shimmer": "Мягкий женский голос (Shimmer)",
            "verse": "Четкий нейтральный голос (Verse)",
        }
    ...
    async def synthesize(self, text: str, voice: str = "nova") -> bytes:
        ...
        try:
            ...
            if self.model == "gpt-4o-mini-tts" and self.default_instructions:
                payload["instructions"] = self.default_instructions
                ...
                async with session.post(
                    ...
                    if self.model != self.fallback_model:
                        ...
                        async with session.post(
                            ...
                            raise Exception(
                                "OpenAI TTS fallback error: "
                                f"{fallback_response.status} {fallback_error}"
                            )
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        raise Exception(
                            f"ElevenLabs TTS error: {response.status} {error_text}"
                        )
        ...
        for file_path in self.temp_dir.glob("*.mp3"):
            if current_time - file_path.stat().st_mtime > max_age_seconds:
                try:
                    ...
                    logger.warning(
                        "temp_file_cleanup_error", path=str(file_path), error=str(e)
                    )


# Фабрика для создания TTS сервиса
async def create_tts_service(
    provider_name: str = "openai", redis_client=None
) -> TextToSpeechService:
    """
